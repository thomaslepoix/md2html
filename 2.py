...
# Enphasis
if multiline_emphasis:
	in_text = re.sub(r'__(.*?)__|\*\*(.*?)\*\*', r'<strong>\g<1>\g<2></strong>', in_text, flags=re.S)
	in_text = re.sub(r'_(.*?)_|\*(.*?)\*', r'<em>\g<1>\g<2></em>', in_text, flags=re.S)
else:
	in_text = re.sub(r'__(.*?)__|\*\*(.*?)\*\*', r'<strong>\g<1>\g<2></strong>', in_text, flags=re.M)
	in_text = re.sub(r'_(.*?)_|\*(.*?)\*', r'<em>\g<1>\g<2></em>', in_text, flags=re.M)
